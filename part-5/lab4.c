...
void *src_start_address = NULL;
void *src_end_address = NULL;
void *dst_start_address = NULL;
void *dst_end_address = NULL;
...
BasicBlock *FindBasicBlock(void *address)
{
...
BasicBlock *FindBasicBlockForTransition(void *address)
{
  BasicBlock *current = basic_blocks;
  BasicBlock *prev = NULL;
...
BasicBlock *RecordBasicBlock(void *startAddress, void *endAddress, uint32_t instruction_count)
{
  // BasicBlock *current = basic_blocks;
  // BasicBlock *prev = NULL;
  ...
  BasicBlock *block = FindBasicBlock(startAddress);
  if (block)
  {
    block->execCount++;
    return block;
...
void AddTransition(void *src_start, void *src_end, void *dst_start, void *dst_end)
{
  Transition *current = transitions;
  Transition *prev = NULL;
  while (current)
  {
    if (current->src_start_addr == src_start && current->src_end_addr == src_end && current->dst_start_addr == dst_start && current->dst_end_addr == dst_end)
    {
      current->label++;
      return;
    }
  ...
  if (!newTransition)
  {
    perror("Failed to allocate memory for transition");
    exit(1);
  }
  newTransition->src_start_addr = src_start;
  newTransition->src_end_addr = src_end;
  newTransition->dst_start_addr = dst_start;
  newTransition->dst_end_addr = dst_end;
  newTransition->label = 1;
...
void PrintBasicBlockStatistics(void)
{
  ...
  if (dotFile == NULL)
  {
    perror("Failed to open file cfg.dot");
    exit(EXIT_FAILURE);
  }

  fprintf(dotFile, "digraph G {\n");
  Transition *trans = transitions;
  while (trans != NULL)
  {
    fprintf(dotFile, "  \"%p ~ %p\" -> \"%p ~ %p\" [label=%d];\n",
            trans->src_start_addr, trans->src_end_addr,
            trans->dst_start_addr, trans->dst_end_addr,
            trans->label);
    trans = trans->next;
  }
  fprintf(dotFile, "}\n");
...
void patchNextInstruction(void *func)
{
  ...
  while (!flag)
  {
    ...
    if (inst->isCflow == 1)
    {
      ...
      dst_start_address = func;
      dst_end_address = func + i;
      AddTransition(src_start_address, src_end_address, dst_start_address, dst_end_address);
      src_start_address = dst_start_address;
      src_end_address = dst_end_address;
      // AddTransition();
      patchedAddr = (func + i);
